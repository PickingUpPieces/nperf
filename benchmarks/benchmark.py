...
def run_test(run_config):
    ...
    client_command = [PATH_TO_NPERF_BIN, 'client', '--output-format=json']
...
def write_results_to_csv(test_results, test_name, csv_file_path):
    ...
    header = ['test_name', 
              'run_number', 
              'run_name', 
              'amount_threads_client', 
              'amount_threads_server', 
              'test_runtime_length', 
              'datagram_size', 
              'packet_buffer_size', 
              'exchange_function', 
              ...
              'uring-sq-mode', 
              'uring-task-work', 
              'amount_uring_multishot_canceled', 
              'amount_uring_copied_zc', 
              'total_data_gbyte', 
              'amount_datagrams', 
              'amount_data_bytes', 
              'amount_reordered_datagrams', 
              'amount_duplicated_datagrams', 
              'amount_omitted_datagrams', 
              'amount_syscalls', 
              'amount_io_model_calls', 
              'data_rate_gbit', 
              'packet_loss', 
              'nonblocking', 
              'ip_fragmentation', 
              'multiplex_port_client', 
    ...
    with open(csv_file_path, 'a', newline='') as csvfile:
        ...
        for index, (server_result, client_result) in enumerate(test_results):
            ...
            row = {
                ...
                'uring-mode': server_result['parameter']['uring_parameter']['uring_mode'],
                ...
                'uring-sq-mode': server_result['parameter']['uring_parameter']['sq_filling_mode'],
                'uring-task-work': server_result['parameter']['uring_parameter']['task_work'],
                'amount_uring_multishot_canceled': server_result['uring_canceled_multishot'],
                'amount_uring_copied_zc': server_result['uring_copied_zc'],
