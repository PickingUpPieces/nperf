...
fn sendmsg_recvmsg() -> Result<(), Box<dyn std::error::Error>>{
    let handle = common::start_nperf_server(Some(vec!["--with-msg".to_string(), "--port".to_string(), "45001".to_string()]));
...
fn sendmmsg_recvmmsg() -> Result<(), Box<dyn std::error::Error>>{
    let handle = common::start_nperf_server(Some(vec!["--with-mmsg".to_string(), "--port".to_string(), "45003".to_string()]));
...
fn sendmsg_recvmmsg() -> Result<(), Box<dyn std::error::Error>>{
    let handle = common::start_nperf_server(Some(vec!["--with-mmsg".to_string(), "--port".to_string(), "45004".to_string()]));
