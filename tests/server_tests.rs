...
fn test_server_send() -> Result<(), Box<dyn std::error::Error>>{
    let handle = common::start_nperf_client(Some(vec!["--port".to_string(), "45001".to_string()]));
...
fn test_server_sendmsg() -> Result<(), Box<dyn std::error::Error>>{
    let handle = common::start_nperf_client(Some(vec!["--port".to_string(), "45002".to_string()]));
...
fn test_server_sendmmsg() -> Result<(), Box<dyn std::error::Error>>{
    let handle = common::start_nperf_client(Some(vec!["--port".to_string(), "45003".to_string()]));
