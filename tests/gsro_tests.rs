...
fn gro_no_gso() -> Result<(), Box<dyn std::error::Error>>{
    let handle = common::start_nperf_server(Some(vec!["--with-msg".to_string(), "--with-gro".to_string(), "--port".to_string(), "45001".to_string()]));
...
fn gso_gro() -> Result<(), Box<dyn std::error::Error>>{
    let handle = common::start_nperf_server(Some(vec!["--with-msg".to_string(), "--with-gro".to_string(), "--port".to_string(), "45003".to_string()]));
