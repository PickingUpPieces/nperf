...
fn test_client_send() -> Result<(), Box<dyn std::error::Error>>{
    common::start_nperf_server(Some(vec!["--port".to_string(), "45001".to_string()]));
...
fn test_client_sendmsg() -> Result<(), Box<dyn std::error::Error>>{
    common::start_nperf_server(Some(vec!["--port".to_string(), "45002".to_string()]));
...
fn test_client_sendmmsg() -> Result<(), Box<dyn std::error::Error>>{
    common::start_nperf_server(Some(vec!["--port".to_string(), "45003".to_string()]));
